...
class CreatParticlesInsideOfADomain():
    ...
    def inverse_cdf(self, u):
        if u <= self.F_1_0_3:
            return u * (self.trunc_num**3) / self.k1
        else:
            u_adjusted = (u - self.F_1_0_3) / self.k2
            return (1 / (1 / self.trunc_num**2 - 2 * u_adjusted))**0.5
    ...
    def CreatParticles(self, RVE_size):
        ...
        self.trunc_num = 0.5
        self.k1 = self.trunc_num**3
        self.k2 = 2 / (1 / self.trunc_num**2 - 1)
        
        self.F_1_0_3 = self.k1 / self.trunc_num**3 * self.trunc_num
        self.F_2_0_3 = self.k2 * (-1 / (2 * self.trunc_num**2) + 1 / 2) + self.F_1_0_3
        ...
        while particle_volume < aim_volume:
            ...
            if is_first_particle:
                ...
                if self.parameters_all["periodic_boundary_option"].GetBool():
                    ...
                    c_x = self.inverse_cdf(u)
                    x = c_x * ((self.x_max - self.x_min)*0.5) * random.choice([-1, 1])
                    inverse_weight = abs(x) / ((self.x_max - self.x_min)*0.5)
                    if inverse_weight < self.trunc_num:
                        inverse_weight = self.trunc_num
                        r = inverse_weight * r_origin
                    else:
                        r = inverse_weight * r_origin
                else:
                    ...
                    u = random.uniform(0, 1)
                    c_x = self.inverse_cdf(u)
                    x = c_x * ((self.x_max - self.x_min)*0.5 - radius_max) * random.choice([-1, 1])
                    inverse_weight = abs(x) / ((self.x_max - self.x_min)*0.5 - radius_max)
                    if inverse_weight < self.trunc_num:
                        inverse_weight = self.trunc_num
                        r = inverse_weight * r_origin
                    else:
                ...
                while IsOverlaped:
                    ...
                    if self.parameters_all["periodic_boundary_option"].GetBool():
                        ...
                        #inverse_weight = ((self.x_max - abs(x)) / ((self.x_max - self.x_min)*0.5))**(1/3)
                        u = random.uniform(0, 1)
                        c_x = self.inverse_cdf(u)
                        self.x = c_x * ((self.x_max - self.x_min)*0.5) * random.choice([-1, 1])
                        inverse_weight = abs(self.x) / ((self.x_max - self.x_min)*0.5)
                        if inverse_weight < self.trunc_num:
                            inverse_weight = self.trunc_num
                            r = inverse_weight * r_origin
                        else:
                        ...
                        u = random.uniform(0, 1)
                        c_x = self.inverse_cdf(u)
                        self.x = c_x * ((self.x_max - self.x_min)*0.5 - radius_max) * random.choice([-1, 1])
                        inverse_weight = abs(self.x) / ((self.x_max - self.x_min)*0.5 - radius_max)
                        if inverse_weight < self.trunc_num:
                            inverse_weight = self.trunc_num
                            r = inverse_weight * r_origin
                        else:
