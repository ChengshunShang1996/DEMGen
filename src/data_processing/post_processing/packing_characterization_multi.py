...
class PackingCharacterizationMulti(PackingCharacterization):
    ...
    def CreateInitialCases(self):
        ...
        while packing_cnt <= packing_num:
            cases_folder_name = "case_" + str(packing_cnt)
            self.clear_old_cases_folder(cases_folder_name)
            self.copy_seed_files_to_aim_folders(cases_folder_name)
            packing_cnt += 1
    ...
    def RunDEM(self):
        ...
        while packing_cnt <= packing_num:
            ...
            aim_path = os.path.join(current_path, "generated_cases", aim_folder_name, cases_folder_name)
            os.chdir(aim_path)
            if os.name == 'nt': # for windows
                subprocess.run(['python', 'packing_characterization_run.py'], check=True)
            else: # for linux
                subprocess.run(['python3', 'packing_characterization_run.py'], check=True)
            packing_cnt += 1

    def clear_old_cases_folder(self, cases_folder_name):

        aim_folder_name = 'packing_characterization'

        if self.parameters["generator_type"] =="constructive":
            cases_folder_path = aim_folder_name
        elif self.parameters["generator_type"] =="dynamic":    
            cases_folder_path = os.path.join(os.getcwd(), 'generated_cases', cases_folder_name, aim_folder_name)
    ...
    def copy_seed_files_to_aim_folders(self, cases_folder_name):
        ...
        if self.parameters["generator_type"] =="constructive":
            ...
            aim_path = os.path.join(os.getcwd(), 'generated_cases', cases_folder_name, aim_folder_name)
        ...
        if self.parameters["generator_type"] =="constructive":
            ...
            seed_file_path_and_name = os.path.join(os.getcwd(), 'generated_cases', cases_folder_name, 'show_packing', 'inletPGDEM.mdpa')
            aim_file_path_and_name = os.path.join(aim_path, 'inletPGDEM.mdpa')
            shutil.copyfile(seed_file_path_and_name, aim_file_path_and_name)

            seed_file_path_and_name = os.path.join(os.getcwd(), 'generated_cases', cases_folder_name, 'show_packing', 'ProjectParametersDEM.json')
            aim_file_path_and_name = os.path.join(aim_path, 'ProjectParametersDEM.json')
            with open(seed_file_path_and_name, "r") as f_material:
                with open(aim_file_path_and_name, "w") as f_material_w:
                    for line in f_material.readlines():
